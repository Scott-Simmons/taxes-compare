...
const TaxAmountsPlot: React.FC<TaxAmountsPlotProps> = ({ data }) => {
  ...
  if (data.country_comb_data) {
    Object.entries(data.country_comb_data).flatMap(([country_comb_key, pointsData]) => {
      ...
      const rates = pointsData.breakeven_tax_amounts;
  ...
  if (income) {
    Object.entries(data.country_specific_data).flatMap(([country_key, incomeData]) => {
      const rate = incomeData.specific_tax_amount;
        shapes.push(
          {
            type: 'line',
            x0: income,
    ...
    <div style={{
      ...
      <Plot
        ...
        layout={{
          ...
          xaxis: { title: `<b>Annual Taxable Income</b> (in ${currency || "local currency"})`, gridcolor: 'rgba(255, 255, 255, 0)', range: [0, undefined] },
          yaxis: { title: `<b>Taxation Amount</b> (in ${currency || "local currency"})`, gridcolor: 'rgba(255, 255, 255, 0)', range: [0, undefined] },
          showlegend: true,
